...
#' Checks if the value is a directory
#'
#' @param value the value to check
#' @param allow_null if TRUE, allow the value NULL. If FALSE (default) do not
#'                   allow NULL.
...
#' \dontrun{
#' # For assertion
#' assertthat::assert_that(qscheck::is_path(f))
#' # For check
#' if (qscheck::is_path(f)) {}
#' }
#'
#'
#' @concept directory
...
is_path <- function(
  ...
  res <- inspect_is_path(value, allow_null)
  return(res$valid)
}

assertthat::on_failure(is_path) <- function(call, env) {
  value <- callget(call, env, "value", NULL)
  allow_null <- callget(call, env, "allow_null", FALSE)
  res <- inspect_is_path(value, allow_null)
  ...
  msg <- paste0(
    deparse(call$value),
    snippet_must_be("valid directory"),
    snippet_null(allow_null),
    ". ", res$reason
  )
...
inspect_is_path <- function(value, allow_null) {
  if (is.null(value)) {
    if (allow_null) {
      return(success())
    } else {
